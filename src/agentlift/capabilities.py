...
CAPABILITIES = {
    ...
    "openai": {
        ...
        "subagents": {"tier": "degraded",
            "reason": "multi-agent runs only as nodes inside ONE Agent Builder workflow (you call the workflow by id, not each agent); not a roster of separately-deployable, individually-addressable agents",
            "remediation": "compile the roster into one workflow (or workflows-as-tools) - agentlift maps agents 1:1 today; or self-host the roster via export"},
