...
- `deploy` — push to a live managed runtime via API: **Anthropic** (full) and **Google `--target google`** (preview); **AWS Bedrock `--target bedrock --build-only`** compiles a deployable AgentCore Runtime container (hosted-create manual until live-verified — Claude-native, no model remap).
...
> ARN/id) — to `****`, and never commit the Bedrock bearer token. See
...
The sharpest gap is Bedrock's hosted runtime — `audit` rates it `native` (AgentCore genuinely
hosts agents), but agentlift ships only the **build-only** artifact path and *refuses* the
unverified hosted-create. Both true, not contradictory.

| | Anthropic | AWS Bedrock (`--target bedrock`) | Google (`--target google`) | OpenAI |
|---|---|---|---|---|
| Handoff | `deploy` (live, **full**) | `deploy --build-only` (**build-only preview** — ARM64 AgentCore Runtime container; hosted create manual) | `deploy` (live, **preview**) | `export` + self-host only |
| Subagents | native, per-agent IDs | emulated (one AgentCore runtime, Strands agents-as-tools in-model delegation) | emulated (one `reasoningEngine`, server-side delegation) | `as_tool`, loop in your app |
| Skills | uploaded, shared by id (skill-bearing agents auto-get `read` — Managed Agents needs it to open `SKILL.md`) | ✅ embedded in source package, loaded via Strands `Skill.from_file` + `AgentSkills` (update = redeploy) | ✅ embedded in source package, loaded via ADK `load_skill_from_dir` (update = redeploy) | export comment only |
| Remote MCP | mapped | ✅ URL → Strands `MCPClient` (streamable-HTTP) + `tool_filter`; inline auth → AgentCore Runtime `env_vars` (resolved at deploy, never inlined) | ✅ URL → ADK `McpToolset` + `tool_filter`; inline auth → Agent Engine `env_vars` (resolved at deploy, never inlined) | export comment only |
| Built-in web tools (`web_search`/`web_fetch`) | mapped | 🚧 PLANNED (`web_fetch`→Browser tool; no hosted `web_search` primitive) | ✅ `web_search`→Google Search grounding, `web_fetch`→URL Context, each a wrapped single-tool ADK sub-agent (`AgentTool`, `propagate_grounding_metadata=True`); always-wrap so they coexist with `transfer_to_agent`; pins `google-adk>=1.34.3` | `WebSearchTool` / self-host fetch |
| Built-in sandbox tools (`bash/files/glob-grep`) | mapped | 🚧 PLANNED — a *real* AgentCore Code Interpreter (shell + FS) + Browser exist (audit: `emulated`, **not** a non-goal); not yet wired, expose via URL MCP meanwhile | 🚧 skipped (sandbox is Python/JS only — in-engine emulation is a **non-goal**; expose equivalents via a URL MCP server) | self-host runner |
| `:ask` | permission policy | ❌ unsupported (hosted `/invocations` has no interactive approval channel; gate client-side) | 🚧 unsupported on `VertexAiSessionService` (gate client-side, or keep on Anthropic) | client-side |
| Idempotency | lockfile + content hashes | ✅ `.agentlift-bedrock.json` spec hash → create/update/skip | ✅ `.agentlift-google.json` spec hash → create/update/skip | n/a |
| Model mapping | Claude (native) | ✅ **Claude-native mapping — no remap**; `claude-*` → regional Bedrock inference profile (`eu.anthropic.claude-haiku-4-5-…` in `eu-north-1`). Mapping fact, not proven inference: live Claude composition receipt **pending Gate A** (composition itself live-proven on Nova) | 🔁 mapped to Gemini (`gemini-2.5-flash`); Claude-on-Vertex is an offline-verified **spike, not shipped** (`experiments/claude-on-vertex/`) — a Claude `--google-model` is refused (`google.deploy_model.claude_unsupported`) | 🔁 mapped to `gpt-*` |
...
**Bedrock status (build-only preview):** `build_bedrock_plan` is pure + offline-tested; only
`bedrock_target.py` touches the network, and a **bare** `deploy --target bedrock` (hosted
create) **refuses** — the AgentCore control-plane `create_agent_runtime` wire shape is not
live-verified here, so it's manual (same *confirm-live-before-encoding* rule that gates
Claude-on-Vertex). `--build-only` materializes the full ARM64 container build context under
`<path>/.agentlift-build/bedrock/`. **Two one-time gates outside the code path:** Gate A = the
Anthropic use-case form (Bedrock console → Model access → Anthropic) — a per-account
entitlement for Claude inference that is **eventually consistent** (it cleared once in testing,
then flapped back to `ResourceNotFoundException`); Gate B = AWS IAM creds + execution role +
ECR for the hosted create. The Strands **composition is proven live** on Amazon Nova Pro
(objective tool-call trace, bearer-token model inference — `experiments/bedrock-composition/`);
the same-Claude-brain receipt is pending stable Gate A. The model map is **native** (no
Gemini-style remap): folder `claude-*` → `<region_prefix>.anthropic.<slug>` (eu-*→eu, us-*→us,
ap-*→apac, else→global). See [docs/deploy-bedrock.md](docs/deploy-bedrock.md).

## Commands

```bash
agentlift validate <path>              # parse + plan, report problems (exit 1 on errors)
agentlift plan     <path> [--json] [--target anthropic|bedrock|google] [--google-model M]  # deterministic deploy plan, no network
...
agentlift deploy   <path> [--target anthropic|bedrock|google] [--build-only] [--prune] [-y]   # --build-only required for bedrock
...
- **AWS Bedrock identifiers, same rule:** the **Bedrock bearer token** (`AWS_BEARER_TOKEN_BEDROCK`,
  ...
  ARN/id** to `****` in tracked files. The build-only artifact uses `<acct>` placeholders by design;
  the model id / region / spec-hash are not identifying and may stay. Live state can leak real values
  into `.agentlift-build/` and any console paste — re-anonymize before staging.
- **Sanity check before committing:** `git grep -nE "gen-lang-client-|reasoningEngines/[0-9]|projects/[0-9]{6,}|[0-9]{12}\.dkr\.ecr|AWS_BEARER_TOKEN_BEDROCK=.+|ABSK[A-Za-z0-9]"`
...
- [docs/deploy-bedrock.md](docs/deploy-bedrock.md) — AWS Bedrock AgentCore: bearer-token vs IAM, the two gates, the build-only artifact path
